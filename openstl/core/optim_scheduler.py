...
def get_optim_scheduler(args, epoch, model, steps_per_epoch):
    ...
    if hasattr(args, "total_steps") and int(getattr(args, "total_steps", 0)) > 0:
    ...
    if sched_lower == 'onecycle':
        onecycle_kwargs = dict(
            optimizer=optimizer,
            max_lr=args.lr,
        )
        if total_steps > 0:
            onecycle_kwargs["total_steps"] = total_steps
        else:
            onecycle_kwargs["steps_per_epoch"] = int(steps_per_epoch)
            onecycle_kwargs["epochs"] = int(epoch)

        if hasattr(args, "pct_start"):        onecycle_kwargs["pct_start"] = args.pct_start
        if hasattr(args, "div_factor"):       onecycle_kwargs["div_factor"] = args.div_factor
        if hasattr(args, "final_div_factor"): onecycle_kwargs["final_div_factor"] = args.final_div_factor

        lr_scheduler = optim.lr_scheduler.OneCycleLR(**onecycle_kwargs)
        by_epoch = False
