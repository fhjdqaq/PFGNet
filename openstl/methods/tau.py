...
class TAU(SimVP):
    ...
    def training_step(self, batch, batch_idx):
        ...
        loss_main = self.criterion(pred_y, batch_y)
        loss = loss_main + self.hparams.alpha * self.diff_div_reg(pred_y, batch_y)

        self.log('train_loss_step', loss,
                 on_step=True, on_epoch=False, prog_bar=True, logger=True, sync_dist=True)

        if hasattr(self, 'train_loss_epoch_metric'):
            self.train_loss_epoch_metric.update(loss.detach())
        else:
            self._train_sum += float(loss.detach().item())
            self._train_cnt += 1
