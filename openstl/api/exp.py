...
class BaseExperiment(object):
    ...
    def _load_callbacks(self, args, save_dir, ckpt_dir):
        ...
        is_main = (not self._dist) or (not dist.is_available()) or (not dist.is_initialized()) or (dist.get_rank() == 0)
        if is_main and (not self.args.no_display_method_info):
            method_info = self.display_method_info(args)
        ...
        ckpt_callback = BestCheckpointCallback(
            ...
            #filename='best-{epoch1:0f}-{val_loss:.3f}',
            filename='best-{epoch:.0f}-{val_loss:.3f}',
            mode='min',
            save_last=True,
            dirpath=ckpt_dir,
            verbose=True,
    ...
    def display_method_info(self, args):
        ...
        if args.method in ['simvp', 'tau', 'mmvp', 'wast','pfg']:
